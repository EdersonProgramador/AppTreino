...
export function AiScreen() {
  const { aiPlans, profile, session, refresh, publicConfig } = useStudent();
  const navigation = useNavigation();
  const { st } = useSt();
  const styles = useMenuStyles();
  ...
  if (!moduleOn(publicConfig, "module_ai")) {
    return (
      <StudentPage>
        <BackChip label="Menu" onPress={() => navigation.goBack()} />
        <EmptyState icon="sparkles-outline" title="Coach IA indisponível" text="A academia desativou este módulo." />
      </StudentPage>
    );
  }
